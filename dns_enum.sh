...
read -p "Customer INTERNAL DNS Server IP no subnetmask: " NS
...
if [ -s /root/input/ipint.txt ]; then
  cat /root/output/list/dnsup.txt | cut -d . -f 1,2,3 | sort -u | sed 's/$/.0\/24/' >/root/input/ipint.txt
else
  cat /root/output/list/dnsup.txt | cut -d . -f 1,2,3 | sort -u | sed 's/$/.0\/24/' >/root/input/dnsipint.txt
fi
